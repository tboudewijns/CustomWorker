...
$PVSExtension=".pvs";
$JPGExtension=".jpg";
$PVSmakeCommand="C:\\ptc\\creo_view_adapters_80\\bin\\pvsmake.exe";
$PVSthumbCommand="C:\\ptc\\creo_view_adapters_80\\pvsthumb_setup\\pvsthumbbatch.bat";
$JPGPDFCommand="C:\\ptc\\custom_workers\\AutoCADWorker\\jpeg2pdf.exe";
$ACADCommand="C:\\ThirdPartyApps\\Autodesk\\AutoCAD 2021\\accoreconsole.exe";
$ACADScript="C:\\ptc\\custom_workers\\AutoCADWorker\\GenericWorker_pdf.scr";
...
$pvsfilename=join "",$temp_dir_name,"\\",$sw_object,"_",$sw_ext,$PVSExtension;
$jpgfilename=join "",$temp_dir_name,"\\",$sw_object,"_",$sw_ext,$JPGExtension;
$pdffilename=join "",$temp_dir_name,"\\",$sw_object,".pdf";

print LOGFILE "Converting : $convert_name to PDF, pdf name $pdffilename\n";

#we need to run pvsmake to create a pvs file
#system ("$PVSmakeCommand -p \"$temp_dir_name\" \"$convert_name\"");
#We run pvsthumb to create a jpg
#system ("$PVSthumbCommand -p \"$temp_dir_name\" \"$pvsfilename\"");
#Now convert the jpg to pdf
#system ("C: && cd \"$temp_dir_name\" && $JPGPDFCommand -o \"$pdffilename\" \"$jpgfilename\"");
#Lets run the autocad console command
system ("\"$ACADCommand\" \/i \"$convert_name\" \/s  \"$ACADScript\"");
#let us run pvsmake to create a pvs file
system ("$PVSmakeCommand -p \"$temp_dir_name\" \"$pdffilename\"");

print LOGFILE "\"$ACADCommand\" \/i \"$convert_name\" \/s  \"$ACADScript\"\n";
...
$ed_file_name=join "",$temp_dir_name_two,"\\",$sw_object,"_pdf.pvs";
#replace spaces with _
$ed_file_name=~ s/ /\_/g;
print LOGFILE "and generating : $ed_file_name\n";
#We run pvsthumb to create a jpg
system ("$PVSthumbCommand -p \"$temp_dir_name\" \"$ed_file_name\"");
print LOGFILE "Generated PVSThumb Thumbnail\n";
...
#print LOGFILE " Executing \"D:\\ptc\\custom_workers\\AutoCADWorker\\doc2pdf.vbs\" \"$jpgfilename\"\n";
#system ("\"C:\\ptc\\custom_workers\\AutoCADWorker\\doc2pdf.vbs\" \"$jpgfilename\"");
...
#system ("copy $output_file D:\\ptc\\custom_workers\\AutoCADWorker\\convert");
...
#remove dwg and pvs and jpg file
	system ("del /F \"$convert_name\"");
	#system ("del /F \"$pvsfilename\"");
	#system ("del /F \"$jpgfilename\"");
  } else {
